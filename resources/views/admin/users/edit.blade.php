...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Update informasi pengguna: <span class="font-semibold text-gray-700">{{ $user->name }}</span></p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.users.update', $user->id) }}" enctype="multipart/form-data" id="userForm">
            ...
            <div class="bg-gradient-to-br from-gray-50 to-gray-100 p-6 border-b border-gray-200">
                <div class="flex flex-col items-center">
                        ...
                        <div id="imagePreviewContainer" class="relative">
                            @if($user->image)
                                <img id="currentImage" 
                                     ...
                                     alt="{{ $user->name }}"
                                     class="w-32 h-32 rounded-full object-cover shadow-lg ring-4 ring-white transition-all group-hover:scale-105">
                            @else
                                <div id="currentImage" class="w-32 h-32 rounded-full bg-gradient-to-br from-[#D32F2F] to-[#B71C1C] flex items-center justify-center text-white text-4xl font-bold shadow-lg ring-4 ring-white transition-all group-hover:scale-105">
                                    {{ strtoupper(substr($user->name, 0, 2)) }}
                                </div>
                            @endif
                            <img id="imagePreview" class="hidden w-32 h-32 rounded-full object-cover shadow-lg ring-4 ring-white" alt="Preview">
                        </div>
                    ...
                    <p class="text-sm text-gray-600 mt-3">Update Foto Profil</p>
                    <p class="text-xs text-gray-500 mt-1">JPG, PNG, JPEG (Max: 2MB) - Kosongkan jika tidak ingin mengubah</p>
                    @error('image')
                    ...
                    @if($user->image)
                        <button type="button" 
                                id="removeImageBtn"
                                class="mt-3 text-xs text-red-600 hover:text-red-700 font-medium flex items-center gap-1">
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="border-t border-gray-200 pt-6">
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-5 flex items-start gap-2">
                        ...
                        <p class="text-sm text-blue-800">Kosongkan field password jika tidak ingin mengubah password</p>
                    </div>
...
<script>
    ...
    document.getElementById('removeImageBtn')?.addEventListener('click', function() {
        ...
        const initials = '{{ strtoupper(substr($user->name, 0, 2)) }}';
        const placeholder = document.createElement('div');
        placeholder.id = 'currentImage';
        placeholder.className = 'w-32 h-32 rounded-full bg-gradient-to-br from-[#D32F2F] to-[#B71C1C] flex items-center justify-center text-white text-4xl font-bold shadow-lg ring-4 ring-white transition-all group-hover:scale-105';
