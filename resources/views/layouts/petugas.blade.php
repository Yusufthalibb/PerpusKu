...
<head>
    ...
    <title>@yield('title', 'Petugas - PerpusKu')</title>
            ...
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-show="open" @click.away="open=false"
                    ...
                    <a href="{{ route('petugas.profile') }}"
                        ...
                        <svg class="w-5 h-5 text-gray-400 {{ request()->routeIs('petugas.profile') ? 'text-[#D32F2F]' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex flex-col h-full">
            ...
            <nav class="flex-1 p-4 lg:p-3 space-y-2 lg:space-y-1">
                <a href="{{ route('petugas.dashboard') }}"
                    ...
                    {{ request()->routeIs('petugas.dashboard') ? 'bg-gray-50 text-[#D32F2F]' : 'text-gray-600 hover:bg-gray-50' }}">
                    <svg class="w-6 h-6 lg:w-5 lg:h-5 {{ request()->routeIs('petugas.dashboard') ? 'text-[#D32F2F]' : 'text-gray-400 group-hover:text-[#D32F2F]' }}" 
                ...
                <a href="{{ route('petugas.books.index') }}"
                    class="flex items-center space-x-3 px-5 py-4 lg:px-4 lg:py-3 rounded-lg group transition-all
                    {{ request()->routeIs('petugas.books.*') ? 'bg-gray-50 text-[#D32F2F]' : 'text-gray-600 hover:bg-gray-50' }}">
                    <svg class="w-6 h-6 lg:w-5 lg:h-5 {{ request()->routeIs('petugas.books.*') ? 'text-[#D32F2F]' : 'text-gray-400 group-hover:text-[#D32F2F]' }}" 
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('petugas.categories.index') }}"
                    class="flex items-center space-x-3 px-5 py-4 lg:px-4 lg:py-3 rounded-lg group transition-all
                    {{ request()->routeIs('petugas.categories.*') ? 'bg-gray-50 text-[#D32F2F]' : 'text-gray-600 hover:bg-gray-50' }}">
                    <svg class="w-6 h-6 lg:w-5 lg:h-5 {{ request()->routeIs('petugas.categories.*') ? 'text-[#D32F2F]' : 'text-gray-400 group-hover:text-[#D32F2F]' }}" 
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('petugas.borrowings.requests') }}"
                    class="flex items-center space-x-3 px-5 py-4 lg:px-4 lg:py-3 rounded-lg group transition-all
                    {{ request()->routeIs('petugas.borrowings.requests') ? 'bg-gray-50 text-[#D32F2F]' : 'text-gray-600 hover:bg-gray-50' }}">
                    <svg class="w-6 h-6 lg:w-5 lg:h-5 {{ request()->routeIs('petugas.borrowings.requests') ? 'text-[#D32F2F]' : 'text-gray-400 group-hover:text-[#D32F2F]' }}" 
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4"/>
                    </svg>
                    <span class="font-medium text-base lg:text-sm">Pengajuan peminjaman</span>
                </a>

                <a href="{{ route('petugas.borrowings.index') }}"
                    class="flex items-center space-x-3 px-5 py-4 lg:px-4 lg:py-3 rounded-lg group transition-all
                    {{ request()->routeIs('petugas.borrowings.index') ? 'bg-gray-50 text-[#D32F2F]' : 'text-gray-600 hover:bg-gray-50' }}">
                    <svg class="w-6 h-6 lg:w-5 lg:h-5 {{ request()->routeIs('petugas.borrowings.index') ? 'text-[#D32F2F]' : 'text-gray-400 group-hover:text-[#D32F2F]' }}" 
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                    </svg>
                    <span class="font-medium text-base lg:text-sm">peminjaman Aktif</span>
                </a>

                <a href="{{ route('petugas.borrowings.history') }}"
                    class="flex items-center space-x-3 px-5 py-4 lg:px-4 lg:py-3 rounded-lg group transition-all
                    {{ request()->routeIs('petugas.borrowings.history') ? 'bg-gray-50 text-[#D32F2F]' : 'text-gray-600 hover:bg-gray-50' }}">
                    <svg class="w-6 h-6 lg:w-5 lg:h-5 {{ request()->routeIs('petugas.borrowings.history') ? 'text-[#D32F2F]' : 'text-gray-400 group-hover:text-[#D32F2F]' }}" 
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    <span class="font-medium text-base lg:text-sm">Riwayat peminjaman</span>
                </a>

                <a href="{{ route('petugas.reports.index') }}"
                    class="flex items-center space-x-3 px-5 py-4 lg:px-4 lg:py-3 rounded-lg group transition-all
                    {{ request()->routeIs('petugas.reports.*') ? 'bg-gray-50 text-[#D32F2F]' : 'text-gray-600 hover:bg-gray-50' }}">
                    <svg class="w-6 h-6 lg:w-5 lg:h-5 {{ request()->routeIs('petugas.reports.*') ? 'text-[#D32F2F]' : 'text-gray-400 group-hover:text-[#D32F2F]' }}" 
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-5 lg:p-4 border-t border-gray-100 space-y-3">
                ...
                <div class="grid grid-cols-2 gap-3 lg:gap-2 mb-3">
                    ...
                    <div class="bg-gradient-to-br from-amber-50 to-orange-50 rounded-lg p-4 lg:p-3">
                        <p class="text-sm lg:text-xs text-gray-600 mb-1">Dipinjam</p>
